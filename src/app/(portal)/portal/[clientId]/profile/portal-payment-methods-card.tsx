...
import {
  portalCreatePaymentMethod,
  portalUpdatePaymentMethod,
  portalDeletePaymentMethod,
...
export function PortalPaymentMethodsCard({
  clientId,
  initialMethods,
}: {
  clientId: string
  ...
  function handleAdd(e: React.FormEvent) {
    ...
    startTransition(async () => {
      const pm = await portalCreatePaymentMethod({
        clientId,
        type: addType,
        label: addLabel,
        notes: addNotes,
  ...
  function handleSaveEdit(e: React.FormEvent) {
    ...
    if (!editState) return
    startTransition(async () => {
      const updated = await portalUpdatePaymentMethod(
        editState.id,
  ...
  function handleSetDefault(pm: PaymentMethod) {
    startTransition(async () => {
      await portalUpdatePaymentMethod(pm.id, { isDefault: true }, clientId)
      setMethods((prev) => prev.map((m) => ({ ...m, isDefault: m.id === pm.id })))
    })
  }

  function handleDelete(pm: PaymentMethod) {
    startTransition(async () => {
      await portalDeletePaymentMethod(pm.id, clientId)
          ...
          <form key={pm.id} onSubmit={handleSaveEdit} className="border rounded-md p-3 bg-gray-50 space-y-3">
            <div className="space-y-1">
              ...
              <Select
                ...
                onValueChange={(v) => setEditState((s) => s && { ...s, type: v })}
              >
